...
//			query = session.createQuery("select RT.tableNumber, temp.confirmationNumber from ReservationTable RT, (select tableNumber, confirmationNumber from Reservation where date = :param1 and time = :param2 and reservationStatus = 'CONFIRMED') as temp where RT.tableNumber = temp.tableNumber and RT.tableNumber = :param3");
