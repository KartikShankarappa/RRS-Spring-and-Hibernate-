(function() {
    ...
    reservationService.$inject = ['$q', '$http', '$rootScope', '$cookieStore'];

    function reservationService($q, $http, $rootScope, $cookieStore) {
...
//                .post('http://localhost:8080/RRSRestApp/api/admin/reservations/' + $rootScope.username + '/' + $rootScope.token)
            	.post('http://localhost:8080/RRSRestApp/api/admin/reservations/' + $cookieStore.get('userName') + '/' + $cookieStore.get('token'))
                .then(successFn, errorFn);
